...
export function EventsPage() {
  ...
  const load = useCallback(async () => {
    try { setItems(await eventsService.getAll()); }
  ...
  function openEdit(ev: Event) {
    setEditTarget(ev);
    setForm({ name: ev.name, description: ev.description ?? '', starts_at: ev.starts_at ?? '', ends_at: ev.ends_at ?? '' });
    setFormOpen(true);
  }

  async function openDelete(ev: Event) {
    const count = await eventsService.getBudgetCount(ev.id);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editTarget) {
        const updated = await eventsService.update(editTarget.id, dto);
  ...
  return (
    <div className="settings-page">
      <div className="settings-page__header">
        <h1 className="settings-page__title">{t('settings_nav.events')}</h1>
      ...
      {loading ? <p style={{ color: 'var(--text-secondary)' }}>{t('common.loading')}</p>
        : items.length === 0 ? <div className="settings-empty">{t('events.empty')}</div>
        : (
          <div className="settings-list">
            {items.map((ev) => (
              <div key={ev.id} className="settings-item">
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="settings-item__name">{ev.name}</div>
                  {(ev.starts_at || ev.ends_at) && (
                    <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '2px' }}>
                      {f(ev.starts_at)}{ev.starts_at && ev.ends_at ? ' — ' : ''}{f(ev.ends_at)}
                    </div>
                  )}
                </div>
      ...
      <Modal open={formOpen} title={editTarget ? t('common.edit') : t('events.add')} onClose={() => setFormOpen(false)}
        footer={<>
      ...
      <ConfirmDialog
        ...
        message={deleteBudgets > 0 ? t('events.delete_warning_budgets', { count: deleteBudgets }) : t('events.delete_warning')}
